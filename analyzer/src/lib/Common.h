...
class InvalidAssignCheck {
public:
	InvalidAssignCheck(Value *sk, Value *br) : SCheck(sk), SCBranch(br) {
    auto I = dyn_cast<Instruction>(SCheck);
    if (!I)
      return;
  ...
  unsigned getSCheckLineNo() { return SCheckLineNo; }
	...
	friend bool operator< (const InvalidAssignCheck &SC1, const InvalidAssignCheck &SC2) {
		return (SC1.SCheck < SC2.SCheck);
	}
