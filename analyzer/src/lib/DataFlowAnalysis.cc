		...
		if (CI) {
			...
			if (CaV){
				...
				if (cit != Ctx->CopyFuncs.end()) {
					...
					VBDMaps[V].insert(Src);
					findSourceCV(Src, SourceSet, CVSet, TrackedSet);
					continue;
				}
			}
			//direct call or indirect calls; A is the actual argument in CallInst
			Argument *farg = NULL;
			for (Function *Callee : Ctx->Callees[CI]) {
				if (CI->getNumArgOperands() > Callee->arg_size())
					continue;
				OP << "== funcname:" << Callee->getName() <<  "\n";
				unsigned int count = CI->getNumArgOperands();
				unsigned int index = 0;
				for(; index < count; index++){
					if (CI->getArgOperand(index) == A)
						break;
				}
				if (index == count)
					continue;
				farg = Callee->getArg(index);
				//find the last 
				std::set<Value *> AliasSet;
				getAliasPointers(farg, AliasSet, Ctx->FuncPAResults[Callee]);
...
void DataFlowAnalysis::findSourceCV(Value *V, set<Value *> &SourceSet,                                                                                                                                                                                                                                                                                                                                                                                                                                                           
	...
	// if (VBDMaps.count(V) == 0){
	// 	std::list<Value *> Sourcelst(1, CE->getOperand(0));
	// 	VBDMaps[V] = Sourcelst;
	// }
	// if (VBDMaps.count(V) != 0){
	// 	std::set<Value *> VV;
	...
	// 	VV.insert(VBDMaps[V].begin(), VBDMaps[V].end());
	// 	while (!VV.empty()) {
	...
	if (dyn_cast<ConstantExpr>(V)) {
		...
		VBDMaps[V].insert(CE->getOperand(0));
	...
	if (CI) {
		...
		if (CaV) {
			...
			//direct call, but not the redefined functions
			...
			set<ReturnInst *> rets;
			getRetInst(F, rets);
			for(ReturnInst *I : rets){
				Value *ret = I->getReturnValue();
				VBDMaps[V].insert(ret);
				findSourceCV(ret, SourceSet, CVSet, TrackedSet);
			}
	...
	if (SI) {
		CVSet.insert(V);
		VBDMaps[V].insert(SI->getTrueValue());
		VBDMaps[V].insert(SI->getFalseValue());
...
void DataFlowAnalysis::findEqualUses(Value *start, set<Value *> TrackedSet, set<Value *> &UseSet){
	if (TrackedSet.count(start) != 0)
		return;
	TrackedSet.insert(start);

	for (User *use : start->users()) {
		//StoreInst
		if (StoreInst *SI = dyn_cast<StoreInst>(use)){
			Value * V = SI->getPointerOperand();
			Function *Callee = SI->getParent()->getParent();
			std::set<Value *> AliasSet;
			getAliasPointers(V, AliasSet, Ctx->FuncPAResults[Callee]);

			// To find all stores using the pointer
			// TODO: use alias analysis
			for (Value *A : AliasSet) {
				findEqualUses(A, TrackedSet, UseSet);
			}
		...
		if (ReturnInst *RI = dyn_cast<ReturnInst>(use)){
			...
			for (auto *CI : Ctx->Callers[F]){
				findEqualUses(CI, TrackedSet, UseSet);
			}
		}

		//BinaryOperator
		if(BinaryOperator *BI = dyn_cast<BinaryOperator>(use)){
			UseSet.insert(BI);
		}
	}	
}
