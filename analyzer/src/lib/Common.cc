...
string IOPortOp[3]= {"inl", "inb", "inw"}; //"\"outl ", "\"outb ", "\"outw ",
...
//IO identify (current for io port; need to support mmio and dma)
bool ioOpMatch(std::string &S){

  for(int i = 0; i < sizeof(IOPortOp)/sizeof(IOPortOp[0]); i++){
	//maybe re better
	OP << S << IOPortOp[i] << '\n';
    if(S.find(IOPortOp[i]) != std::string::npos){
      return true;
    }
  }
  return false;
...
bool isIOPort(Value *I){
  if(isa<CallInst>(I)){
    CallInst *CI = dyn_cast<CallInst>(I);
    Value * CV = CI->getCalledValue();
    if(isa<InlineAsm>(CV)){
      //inlineAsm
      InlineAsm * IA = dyn_cast<InlineAsm>(CV);
      string asmstr = IA->getAsmString();
      return ioOpMatch(asmstr);
    }
  }
  return false;
}
...
void printInst(Instruction *CI){
	Function *F = CI->getParent()->getParent();
	std::string str;
	raw_string_ostream ss(str);
	ss << *CI;
	OP << ss.str() << " " << F->getName() << F->getName();
}

Value *getValueByField(Value *St, StringRef fieldName){

}

void getRetInst(Function * F, set<ReturnInst *> &rets) {

    vector<BasicBlock*> bbVec;
    set<BasicBlock*> visited;
    if((!F) || F->isDeclaration())
    	return;
    bbVec.push_back(&F->getEntryBlock());
    while(!bbVec.empty()) {
        BasicBlock* bb = bbVec.back();
        bbVec.pop_back();
        for (BasicBlock::iterator it = bb->begin(), eit = bb->end();
                it != eit; ++it) {
            ReturnInst *t = dyn_cast<ReturnInst>(&*it);
            if(t){
            	rets.insert(t);
            }
        }

        for (succ_iterator sit = succ_begin(bb), esit = succ_end(bb);
                sit != esit; ++sit) {
